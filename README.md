...
立即同步一次数据；同步会基于已有缓存只写入新增 event / SNS，并缓存声优图片到 `MEDIA_ROOT`：
...
```bash
docker compose exec api python scripts/sync_data.py --no-actors --no-images
```

更新声优资料，但不下载声优图片：

```bash
docker compose exec api python scripts/sync_data.py --no-images
```
